...
class BaseConverterProvider implements vscode.WebviewViewProvider {
    ...
    private _calculate(
        _values: Array<{ value: string; base: number }>,
        _operators: string[],
        _functions: Array<{ name: string; innerExpr: string }>,
        functionCode: string,
        resultBase: number,
        tokens: Array<{ type: string; name?: string; value?: string; base?: number; innerExpr?: string; operatorAfter?: string }>
    ): string {
        // Build function results map
        const functionResults = new Map<number, bigint>();

        if (functionCode) {
            const funcMatch = functionCode.match(/def\s+(\w+)\s*\((\w+)\s*(?:,\s*\w+)*\)\s*:\s*return\s+(.+)/);
            if (funcMatch) {
                const [, funcName, paramName, body] = funcMatch;

                // Find all function tokens and evaluate them
                tokens.forEach((tok, idx) => {
                    if (tok.type === 'function' && tok.name === funcName && tok.innerExpr) {
                        const innerTokens = this._parseSimpleExpression(tok.innerExpr);
                        if (innerTokens.length === 1 && innerTokens[0].type === 'value') {
                            ...
                            functionResults.set(idx, result);
                        }
                    }
                });
            }
        ...
        // Build arrays of values and operators from tokens
        ...
        for (let i = 0; i < tokens.length; i++) {
            const tok = tokens[i];
            if (tok.type === 'value') {
                const [num, den] = this._toFractionBigInt(tok.value!, tok.base!);
                allValues.push(num / den);
            } else if (tok.type === 'function' && functionResults.has(i)) {
                allValues.push(functionResults.get(i)!);
            }
            if (tok.operatorAfter) {
                allOperators.push(tok.operatorAfter);
            }
        }
        ...
        for (let i = 0; i < allOperators.length && i + 1 < allValues.length; i++) {
